#ident "$Id: mount_nfs.c,v 1.1 2003/09/09 11:22:10 raven Exp $"
/* ----------------------------------------------------------------------- *
 *   
		...
		const char *options, void *context)
{
  ...
  if (options) {
	...
	comma++;
      ...
      syslog(LOG_DEBUG, MODPREFIX "*comma=%x %c  comma=%p %s cp=%p %s "
	     ...
	     *comma, *comma, comma, comma, cp, cp, 
	     nfsoptions, nfsp, nfsoptions+len, nfsp-nfsoptions, len);
#endif
    ...
    nfsp[-1] = '\0';
    syslog(LOG_DEBUG, MODPREFIX "nfs options=\"%s\", nosymlink=%d", 
    ...
    *colon = ':';
    ...
    if ( nfsoptions && *nfsoptions ) {
      ...
      err = spawnl(LOG_NOTICE, PATH_MOUNT, PATH_MOUNT, "-t", "nfs",
		   SLOPPYOPT "-o", nfsoptions, whatstr, fullpath, NULL);
    } else {

...
static char *dequote(const char *str, int strlen)
{
  ...
  *cp = '\0';
  ...
  syslog(LOG_DEBUG, MODPREFIX "dequote(\"%.*s\") -> %s", origlen, str, ret);
		     ...
		     const char *options)
{
  ...
  syslog(LOG_DEBUG, 
	 MODPREFIX "mounting root %s, mountpoint %s, what %s, fstype %s, options %s\n",
  ...
  if ( !strcmp(fstype, "nfs") ) {
    ...
    rv = do_mount(root, mountpoint, strlen(mountpoint), what, fstype, options);
  }

  if (nonstrict && rv) {
    syslog(LOG_DEBUG, "ignoring failure of non-strict mount");
    return 0;
  }
  return rv;
}
		...
		int name_len, const char *mapent, void *context)
{
  ...
  syslog(LOG_DEBUG, MODPREFIX "expanded entry: %s", pmapent);
  ...
  if (*p == '/') {
	...
	do {
	  ...
	  if (myoptions == NULL) {
	    syslog(LOG_ERR, MODPREFIX "multi concat_options: %m");
	    free(options);
	    free(path);
	    return 1;
	  }
	...
	return 1;
      ...
      syslog(LOG_DEBUG, MODPREFIX "multimount: %.*s on %.*s with options %s",
	     loclen, loc, pathlen, path, myoptions);

      rv = sun_mount(root, name, name_len, path, pathlen, loc, loclen, myoptions);
      free(path);
      free(loc);
      free(myoptions);
    ...
    syslog(LOG_DEBUG, MODPREFIX "core of entry: options=%s, loc=%.*s",
	   options, loclen, loc);
